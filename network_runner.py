...
class NetworkRunner:
    """
    This is a class for training and evaluating neural networks.
    """
    def __init__(self, backbone: str, train: bool) -> None:
        self.seed = 10
    ...
    def load_data(self) -> None:
        if self.train:
            self.train_imgs, self.train_labels = load_data_old(
                self.train_pickle, self.img_width, self.img_height, self.n_channels
            )
            self.val_imgs, self.val_labels = load_data_old(
                self.val_pickle, self.img_width, self.img_height, self.n_channels
            )
        self.test_imgs, self.test_labels = load_data_old(
            self.test_pickle, self.img_width, self.img_height, self.n_channels
        )
    ...
    def create_generators(self) -> None:
        ...
        if self.train:
            self.generators = create_datagen_flow(
                [
            ...
            self.balanced_train_generator = BalancedDataGenerator(
                self.train_imgs,
                self.train_labels,
                train_datagen,
    ...
    def setup_basemodel(self) -> None:
        ...
        if hasattr(self.backbone(), "custom_model"):
            ...
            n_trainable_layers = MODEL_PARAMS[self.backbone.__name__][
                "n_trainable_layers"
            ]
    ...
    def model_build(self) -> None:
        ...
        for layer in MODEL_PARAMS[self.backbone.__name__]["fcn_layers"]:
            x = globals()[layer](
                **MODEL_PARAMS[self.backbone.__name__]["fcn_layers"][layer]
            )(x)
    ...
    def model_compile_fit(self) -> None:
        self.final_model.compile(
            optimizer=optimizers.RMSprop(
                learning_rate=MODEL_PARAMS[self.backbone.__name__]["learning_rate"]
            ),
        ...
        self.history = self.final_model.fit(
            self.balanced_train_generator,  # self.balanced_train_generator, #self.generators["train_generator"]
            validation_data=(
                self.val_imgs,
                self.val_labels,
            ),  # (self.val_imgs, self.val_labels), self.generators["val_generator"],
            steps_per_epoch=MODEL_PARAMS[self.backbone.__name__][
                "steps_per_epoch"
            ],  # num of batches in one epoch
    ...
    def run(self) -> None:
        ...
        with tf.device("/CPU:0"):
            ...
            y_pred = to_categorical(
                np.argmax(trained_model.predict(self.test_imgs), axis=1), 43
            )
            ...
            fig, ax = plot_confusion_matrix(
                conf_mat=conf_matrix, figsize=(6, 6), cmap=plt.cm.Greens
            )
